import { useState } from "react";
import { useNavigate } from "react-router-dom";
import Header from "@/components/layout/Header";
import Footer from "@/components/layout/Footer";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { MapPin, Briefcase, Clock, IndianRupee, Building2, Home, Users, ArrowRight, Search } from "lucide-react";
import { Input } from "@/components/ui/input";

type JobType = "all" | "WFH" | "WFO" | "Hybrid";
type Department = "all" | "BPO" | "EdTech" | "Sales" | "Customer Support" | "Tech";

interface Job {
  id: number;
  title: string;
  company: string;
  location: string;
  experience: string;
  salary: string;
  type: "WFH" | "WFO" | "Hybrid";
  department: Department;
  featured?: boolean;
  description: string;
}

const RolesPage = () => {
  const navigate = useNavigate();
  const [selectedType, setSelectedType] = useState<JobType>("all");
  const [selectedDept, setSelectedDept] = useState<Department>("all");
  const [searchQuery, setSearchQuery] = useState("");

  const jobs: Job[] = [
    {
      id: 1,
      title: "Customer Support Executive",
      company: "Leading EdTech Startup",
      location: "Bangalore",
      experience: "0-2 years",
      salary: "₹3-5 LPA",
      type: "WFH",
      department: "Customer Support",
      featured: true,
      description: "Handle customer queries, provide product support, and ensure customer satisfaction.",
    },
    {
      id: 2,
      title: "Inside Sales Representative",
      company: "Fast-growing SaaS Company",
      location: "Mumbai",
      experience: "1-3 years",
      salary: "₹4-7 LPA",
      type: "Hybrid",
      department: "Sales",
      description: "Drive sales through outbound calls, demos, and relationship building.",
    },
    {
      id: 3,
      title: "Voice Process Associate",
      company: "International BPO",
      location: "Hyderabad",
      experience: "0-1 years",
      salary: "₹2.5-4 LPA",
      type: "WFO",
      department: "BPO",
      description: "Handle inbound/outbound calls for international clients with excellent communication.",
    },
    {
      id: 4,
      title: "Academic Counselor",
      company: "Top EdTech Platform",
      location: "Delhi NCR",
      experience: "2-4 years",
      salary: "₹5-8 LPA",
      type: "Hybrid",
      department: "EdTech",
      featured: true,
      description: "Guide students in their learning journey and help them choose the right courses.",
    },
    {
      id: 5,
      title: "Team Lead - Customer Success",
      company: "Enterprise SaaS",
      location: "Pune",
      experience: "4-6 years",
      salary: "₹8-12 LPA",
      type: "WFO",
      department: "Customer Support",
      description: "Lead a team of customer success managers and drive retention metrics.",
    },
    {
      id: 6,
      title: "Business Development Executive",
      company: "EdTech Unicorn",
      location: "Chennai",
      experience: "1-2 years",
      salary: "₹3.5-6 LPA",
      type: "WFH",
      department: "Sales",
      description: "Identify new business opportunities and build strategic partnerships.",
    },
    {
      id: 7,
      title: "Technical Support Engineer",
      company: "Global Tech Company",
      location: "Bangalore",
      experience: "2-4 years",
      salary: "₹6-10 LPA",
      type: "WFH",
      department: "Tech",
      featured: true,
      description: "Provide technical support and troubleshooting for enterprise software products.",
    },
    {
      id: 8,
      title: "Quality Analyst - BPO",
      company: "Fortune 500 BPO",
      location: "Gurgaon",
      experience: "3-5 years",
      salary: "₹5-8 LPA",
      type: "WFO",
      department: "BPO",
      description: "Monitor call quality, provide feedback, and ensure compliance with standards.",
    },
  ];

  const jobTypes: { value: JobType; label: string; icon: React.ReactNode }[] = [
    { value: "all", label: "All Types", icon: <Briefcase size={16} /> },
    { value: "WFH", label: "Work from Home", icon: <Home size={16} /> },
    { value: "WFO", label: "Work from Office", icon: <Building2 size={16} /> },
    { value: "Hybrid", label: "Hybrid", icon: <Users size={16} /> },
  ];

  const departments: Department[] = ["all", "BPO", "EdTech", "Sales", "Customer Support", "Tech"];

  const filteredJobs = jobs.filter((job) => {
    const typeMatch = selectedType === "all" || job.type === selectedType;
    const deptMatch = selectedDept === "all" || job.department === selectedDept;
    const searchMatch = 
      job.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      job.company.toLowerCase().includes(searchQuery.toLowerCase()) ||
      job.location.toLowerCase().includes(searchQuery.toLowerCase());
    return typeMatch && deptMatch && searchMatch;
  });

  const getTypeIcon = (type: "WFH" | "WFO" | "Hybrid") => {
    switch (type) {
      case "WFH":
        return <Home size={14} />;
      case "WFO":
        return <Building2 size={14} />;
      case "Hybrid":
        return <Users size={14} />;
    }
  };

  const handleApplyJob = (job: Job) => {
    navigate("/apply", { state: { job } });
  };

  return (
    <div className="min-h-screen bg-background">
      <Header />
      <main className="pt-20">
        {/* Hero */}
        <section className="py-16 sm:py-20 bg-hero-gradient relative overflow-hidden">
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute inset-0 opacity-30" style={{
              backgroundImage: "radial-gradient(circle, hsl(var(--primary) / 0.15) 1px, transparent 1px)",
              backgroundSize: "40px 40px"
            }}></div>
          </div>
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="text-center max-w-3xl mx-auto">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-accent border border-primary/20 text-accent-foreground text-sm font-medium mb-6">
                <span className="text-2xl font-bold text-redant-r">R</span>
                <span>— Roles</span>
              </div>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-bold text-foreground mb-6">
                Find Your <span className="text-gradient">Dream Role</span>
              </h1>
              <p className="text-lg text-muted-foreground mb-8">
                Explore verified job opportunities across leading companies. Your next career move is just a click away.
              </p>
              
              {/* Search Bar */}
              <div className="relative max-w-xl mx-auto">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" size={20} />
                <Input
                  placeholder="Search jobs by title, company, or location..."
                  className="pl-12 h-14 text-lg rounded-full border-2 focus:border-primary"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
              </div>
            </div>
          </div>
        </section>

        {/* Filters & Jobs */}
        <section className="py-12 sm:py-16">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            {/* Filters */}
            <div className="mb-8 sm:mb-10 space-y-4">
              <div className="flex flex-wrap justify-center gap-2">
                {jobTypes.map((type) => (
                  <Button
                    key={type.value}
                    variant={selectedType === type.value ? "default" : "outline"}
                    size="sm"
                    onClick={() => setSelectedType(type.value)}
                    className="gap-2"
                  >
                    {type.icon}
                    {type.label}
                  </Button>
                ))}
              </div>
              <div className="flex flex-wrap justify-center gap-2">
                {departments.map((dept) => (
                  <Button
                    key={dept}
                    variant={selectedDept === dept ? "secondary" : "ghost"}
                    size="sm"
                    onClick={() => setSelectedDept(dept)}
                  >
                    {dept === "all" ? "All Departments" : dept}
                  </Button>
                ))}
              </div>
            </div>

            {/* Results Count */}
            <p className="text-center text-muted-foreground mb-8">
              Showing <span className="font-semibold text-foreground">{filteredJobs.length}</span> roles
            </p>

            {/* Jobs Grid */}
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredJobs.map((job, index) => (
                <Card
                  key={job.id}
                  variant="elevated"
                  className={`group hover-lift opacity-0 animate-fade-in-up cursor-pointer ${
                    job.featured ? "ring-2 ring-primary/20" : ""
                  }`}
                  style={{ animationDelay: `${index * 0.05}s` }}
                  onClick={() => handleApplyJob(job)}
                >
                  <CardContent className="p-5">
                    {job.featured && (
                      <Badge className="mb-3 bg-gradient-to-r from-redant-r to-redant-e text-primary-foreground border-0">
                        Featured
                      </Badge>
                    )}
                    
                    <h3 className="text-lg font-semibold text-foreground mb-2 group-hover:text-primary transition-colors line-clamp-2">
                      {job.title}
                    </h3>
                    <p className="text-muted-foreground text-sm mb-4">{job.company}</p>

                    <div className="space-y-2 mb-4">
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <MapPin size={14} className="text-primary shrink-0" />
                        <span>{job.location}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <Clock size={14} className="text-primary shrink-0" />
                        <span>{job.experience}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <IndianRupee size={14} className="text-primary shrink-0" />
                        <span>{job.salary}</span>
                      </div>
                    </div>

                    <div className="flex flex-wrap items-center gap-2 mb-4">
                      <Badge variant="secondary" className="gap-1">
                        {getTypeIcon(job.type)}
                        {job.type}
                      </Badge>
                      <Badge variant="outline">{job.department}</Badge>
                    </div>

                    <Button 
                      className="w-full group/btn" 
                      size="sm"
                      onClick={(e) => {
                        e.stopPropagation();
                        handleApplyJob(job);
                      }}
                    >
                      Apply Now
                      <ArrowRight size={14} className="ml-1 group-hover/btn:translate-x-0.5 transition-transform" />
                    </Button>
                  </CardContent>
                </Card>
              ))}
            </div>

            {filteredJobs.length === 0 && (
              <div className="text-center py-16">
                <p className="text-muted-foreground mb-4 text-lg">No jobs found matching your criteria.</p>
                <Button 
                  variant="outline" 
                  onClick={() => { 
                    setSelectedType("all"); 
                    setSelectedDept("all"); 
                    setSearchQuery("");
                  }}
                >
                  Clear All Filters
                </Button>
              </div>
            )}
          </div>
        </section>
      </main>
      <Footer />
    </div>
  );
};

export default RolesPage;
